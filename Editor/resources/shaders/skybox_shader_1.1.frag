...
layout(set = 1, binding = 0) uniform SkyboxBuffer {
	vec3 resolution;
	float time;
...
float noise(in vec2 v) { 
    return textureLod(noiseTexture, (v+.5)/256., 0.).r;
}
...
void densities(in vec3 pos, out float rayleigh, out float mie) {
	...
	if (mincloudheight < h && h < maxcloudheight) {
        cld = cloud(pos+vec3(xaxiscloud,yaxiscloud, zaxiscloud))*cloudy; //direction and speed the cloud movers
		cld *= sin(3.1415*(h-mincloudheight)/mincloudheight) * cloudy;
	}
	#ifdef cloud2
        float cld2 = 0.;
        if (12e3 < h && h < 15.5e3) {
            cld2 = fnoise(pos*3e-4,t)*cloud(pos*32.0+vec3(27612.3, 0.,-t*15e3));
...
{
    vec3 c = vec3(0.);
    float res = skyboxBuffer.resolution.x*2.5;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
	...
	float AR = skyboxBuffer.resolution.x/skyboxBuffer.resolution.y;
    float M = 1.0; //canvas.innerWidth/M //canvas.innerHeight/M --res
    
    vec2 uvMouse = vec2(0, (t*20) / skyboxBuffer.resolution.y);
    uvMouse.x *= AR;
    
   	vec2 uv0 = (fragCoord.xy / skyboxBuffer.resolution.xy);
    uv0 *= M;
    ...
    vec2 uv1 = (fragCoord.xy / skyboxBuffer.resolution.xy);
    uv1 *=M;
    uv1.x *=AR;
...
void main() {
    ...
    vec4 cood = vec4(gl_FragCoord.xy, 0, 1);
    cood.y = (skyboxBuffer.resolution.y) - cood.y;
    //cood = ubo.projection * ubo.view * cood;
	mainImage(fragColor, cood.xy);
    outColor = fragColor;
    
    /*if (gl_FragCoord.x < 523 && gl_FragCoord.y < 961 && gl_FragCoord.x > 521 && gl_FragCoord.y > 959)
    {
        float sinT = sin(t);
        debugPrintfEXT("FragCoord: vec2(%f, %f)\n\t fragTime: %f\n\t fragTime sin: %f\n\t fragTime sin clamped: %f\n",
         gl_FragCoord.x, gl_FragCoord.y, t, sinT, clamp(sin(t), 0.0, 1.0));
    }

    outColor = vec4(clamp(sin(t), 0.0, 1.0), 0.0, 0.0, 1.0);*/
}